...
function render() {
  ...
  setText("heroLede", t(
    "해양·습도·생체 안에서도 출력이 죽지 않는 마찰전기 소재와, 그 소자를 예측하는 디지털 트윈.",
    "Triboelectric materials that hold output in marine, humid, and in-body settings — and a digital twin that predicts the device."
  ));
  setText("spinLabel", t("지금 다루는 축", "Now staging"));
  setText("stageCap", t("접촉-분리 TENG 모식. 장식 애니메이션이며 실험 데이터가 아닙니다.", "Contact-separation TENG schematic. Decorative — not experimental data."));
  spinWords(t(
    ["자가발전 하베스팅","지능형 바이오전자","기능성 나노소재","AI 디지털 트윈"],
    ["self-powered harvesting","intelligent bioelectronics","functional nanomaterials","AI digital twin"]
  ));
  ...
  [["researchH","연구","Research"],["researchSub","공식 교수소개의 네 축. 클릭하면 상세가 바뀝니다.","Four official pillars. Click a card to stage the brief."],["workH","성과","Selected work"],["workSub","공개 Scholar 목록. 연도와 저자 역할로 걸러집니다.","Public Scholar record. Filter by year and authorship."],["piH","책임교수","Principal investigator"],["piSub", t(p.nameKo, p.nameEn) + " · " + t(p.titleKo, p.titleEn), ""],["eduH","학력","Education"],["careerH","경력","Appointments"],["joinH","합류","Join"],["joinSub","학위과정·박사후 과정 문의.","Graduate and postdoctoral inquiries."],["newsH","소식","News"],["contactH","연락","Contact"],["patentH","특허","Patents"],["firstLabel","1저자만","First author only"]].forEach(([id, ko, en]) => setText(id, t(ko, en)));
  [["navResearch","연구","Research"],["navWork","성과","Work"],["navPi","PI","PI"],["navJoin","합류","Join"],["navContact","연락","Contact"]].forEach(([k, ko, en]) => {
  ...
  renderPillars(); renderFeatured(); renderYearChips(); renderPubs();
}

let spinTimer = null;
function spinWords(words) {
  const node = el("spinWord");
  if (!node || !arr(words).length) return;
  let i = 0;
  ...
  spinTimer = setInterval(() => {
    ...
    setTimeout(() => { node.textContent = words[i]; node.style.opacity = "1"; }, 180);
  }, 2200);
}
function metric(v, label) {
  return `<div class="metric"><b>${v ?? "–"}</b><span>${label}</span></div>`;
}
function renderPillars() {
  const items = arr(state.data.research);
  setHtml("pillarNav", items.map((item, i) => `<button class="pillar${i === state.pillar ? " active" : ""}" type="button" data-i="${i}"><small>0${i + 1}</small><b>${t(item.ko, item.en)}</b></button>`).join(""));
  document.querySelectorAll(".pillar").forEach((btn) => {
    btn.addEventListener("click", () => { state.pillar = Number(btn.dataset.i); renderPillars(); });
  });
...
function renderFeatured() {
  ...
  setHtml("featured", pool.map((p) => `<article class="feat"><div class="yr">${p.year}${p.role ? " · " + p.role : ""}</div><a href="${p.url || "#"}">${p.title}</a><p>${p.venue}${typeof p.cites === "number" ? " · cited " + p.cites : ""}</p></article>`).join(""));
}
function renderYearChips() {
  const years = ["all", ...new Set(arr(state.data.publications).map((p) => String(p.year)))];
  setHtml("yearChips", years.map((y) => `<button class="chip${state.year === y ? " on" : ""}" type="button" data-y="${y}">${y === "all" ? t("전체", "All") : y}</button>`).join(""));
  document.querySelectorAll(".chip").forEach((btn) => {
    btn.addEventListener("click", () => { state.year = btn.dataset.y; renderYearChips(); renderPubs(); });
  });
}
...
function field() {
  ...
  const tick = () => {
    ...
    ctx.strokeStyle = "rgba(110,231,197,.22)";
    ctx.fillStyle = "rgba(240,215,160,.8)";
    dots.forEach((a, i) => {
      a.vx += (mouse.x - a.x) * 0.00003; a.vy += (mouse.y - a.y) * 0.00003; a.x += a.vx; a.y += a.vy;
